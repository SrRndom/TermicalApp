...
public final class Regex implements Serializable {
    ...
    */
    ...
    public final List<String> split(CharSequence input, int limit) {
        Intrinsics.checkParameterIsNotNull(input, "input");
        if (!(limit >= 0)) {
            throw new IllegalArgumentException(("Limit must be non-negative, but was " + limit + ClassUtils.PACKAGE_SEPARATOR_CHAR).toString());
        }
        Pattern pattern = this.nativePattern;
        if (limit == 0) {
            limit = -1;
        }
        String[] split = pattern.split(input, limit);
